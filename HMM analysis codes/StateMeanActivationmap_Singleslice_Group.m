function StateMeanActivationmap_Singleslice_Group(path1,iifiles, VOX,DIM, timep,statetc,trialname,thrmin,thrmax,state,scantype);
...
%     % convert 30s og and 15s og
    ...
    if ~exist(strcat(directory,'\State_MeanActivationMap\State_Activation_during\'), 'dir')
        mkdir(strcat(directory,'\State_MeanActivationMap\State_Activation_during\'));
    end
    cd(strcat(directory,'\State_MeanActivationMap\State_Activation_during\'));
...
%                     tmp(iii,jjj,1)=round(structv(iii,jjj,kkk)*32640/max(max(max(structv(iii,jjj,kkk))))/128);
        ...
        writefilename=strcat('MeanActivation_State',num2str(state),'_during_',num2str(thrmin),'-',num2str(thrmax),'.tif');
