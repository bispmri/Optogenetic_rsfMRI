function StateMeanActivationmap_Singleslice_Group_all(path1,iifiles, VOX,DIM, timep,statetc,trialname,thrmin,thrmax,state,scantype);
...
%     % convert 30s og and 15s og
    ...
    if ~exist(strcat(directory,'\State_MeanActivationMap_', num2str(statenum), '\State_Activation_all\'), 'dir')
        mkdir(strcat(directory,'\State_MeanActivationMap_', num2str(statenum), '\State_Activation_all\'));
    end
    cd(strcat(directory,'\State_MeanActivationMap_', num2str(statenum), '\State_Activation_all\'));
...
%         imwrite(uint8(tmp2), strcat(netname, '.tif'), 'tif');
        writefilename=strcat('MeanActivation_State',num2str(state),'_all_',num2str(thrmin),'-',num2str(thrmax),'.tif');
